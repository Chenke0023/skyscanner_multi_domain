...
def test_get_browser_profile_dir_migrates_from_previous_data_root() -> None:
    with TemporaryDirectory() as temp_dir:
        temp_path = Path(temp_dir)
        target_root = temp_path / "runtime" / "browser-profiles"
        previous_data_root = temp_path / "data" / "browser-profiles"
        previous_profile = previous_data_root / "edge-cdp-profile"
        (previous_profile / "Default").mkdir(parents=True)
        ...
        with (
            ...
            patch.object(
                ...
                (temp_path / "outputs", previous_data_root),
            ),
            patch.object(
                app_paths,
                "ensure_runtime_dirs",
